...
1. initialize the agrod node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run agrod with the following flags:
```
agrod --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the agrod node:
 ```
...
To run the eth66 protocol test suite, initialize a agrod node as described above and run the following command,
replacing `<enode>` with the enode of the agrod node:
...
[dns-tutorial]: https://agrod.ethereum.org/docs/developers/agrod-developer/dns-discovery-setup
