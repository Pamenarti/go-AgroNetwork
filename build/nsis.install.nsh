Name "agrod ${MAJORVERSION}.${MINORVERSION}.${BUILDVERSION}" # VERSION variables set through command line arguments
InstallDir "$InstDir"
...
# Install geth binary
Section "agrod" GETH_IDX
  SetOutPath $INSTDIR
  file {{.agrod}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\agrod.exe"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\agrod.exe" "attach"
  createShortCut "$SMPROGRAMS\${APPNAME}\Uninstall.lnk" "$INSTDIR\uninstall.exe"

  # Firewall - remove rules (if exists)
  SimpleFC::AdvRemoveRule "agrod incoming peers (TCP:30303)"
  SimpleFC::AdvRemoveRule "agrod outgoing peers (TCP:30303)"
  SimpleFC::AdvRemoveRule "agrod UDP discovery (UDP:30303)"

  # Firewall - add rules
  SimpleFC::AdvAddRule "agrod incoming peers (TCP:30303)" ""  6 1 1 2147483647 1 "$INSTDIR\agrod.exe" "" "" "Ethereum" 30303 "" "" ""
  SimpleFC::AdvAddRule "agrod outgoing peers (TCP:30303)" ""  6 2 1 2147483647 1 "$INSTDIR\agrod.exe" "" "" "Ethereum" "" 30303 "" ""
  SimpleFC::AdvAddRule "agrod UDP discovery (UDP:30303)" "" 17 2 1 2147483647 1 "$INSTDIR\agrod.exe" "" "" "Ethereum" "" 30303 "" ""

  # Set default IPC endpoint (https://github.com/ethereum/EIPs/issues/147)
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "R" "HKLM" "\\.\pipe\agrod.ipc"
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "A" "HKLM" "\\.\pipe\agrod.ipc"
